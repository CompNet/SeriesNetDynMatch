...
#	stop()
	...
	out.file <- file.path(data.folder,paste0("season",season))
	if(!reload)
	{	com.files <- file.path(data.folder,paste0("*S",sprintf("%02d",season),"E*_cl.txt"))
		treshold <- 0.3
		cmd <- paste0(dyncom.exec," -t ",treshold," -o ",out.file," ",com.files)
	...
	###############################################################################
	cat("\n\n[",format(Sys.time(),"%a %d %b %Y %X"),"] Convert to a meta-graph (season ",season,")\n",sep="")
	if(use.java)
	{	cmd <- paste0("java -Xmx16G -classpath ",dyncompostj.folder," ",dyncompostj.exec," \"",data.folder,"\" ",season)
...
#compare.coms <- function(t1, c1, t2, c2)
#{	com1 <- node.names[all.coms[[t1]][[c1]]]
#	com2 <- node.names[all.coms[[t2]][[c2]]]
#	intcom <- intersect(com1,com2)
#	uncom <- union(com1,com2)
#	jacc <- length(intcom)/length(uncom)
#    cat("size: ",length(com1)," vs. ",length(com2),"\n",sep="")
#    cat("intersection (",length(intcom),"): ",paste(intcom,collapse=","),"\n",sep="")
#    cat("union (",length(uncom),"): ",paste(uncom,collapse=","),"\n",sep="")
#    cat("jaccard: ",jacc,"\n",sep="")
#}
#
...
#{   # build the names of the concerned nodes
#    
